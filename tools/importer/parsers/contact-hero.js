...
/**
 * Parser for hero (base variant) — Contact page.
 * Base: hero. Source: https://fluenceenergy.com/contact/
 * Selector: #main > header.banner
 *
 * This is the BASE hero (full-bleed overlay), identical in shape to the
 * Services/Mosaic page heroes. blocks/hero/hero.js non-carousel branch is the
 * contract: it reads the block's direct child rows as
 ...
 * .banner-title with a p.h4 eyebrow ("Contact Us") and the <h1> ("Ready to
 * solve your energy challenge? Let's talk."). hero.js promotes the first <p>
 * (eyebrow) to .tag-pill.
 */
export default function parse(element, { document }) {
  const cells = [];

  // Row 0: background media — the banner photo (direct-child <img> on the header).
